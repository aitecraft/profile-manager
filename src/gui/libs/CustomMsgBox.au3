...
Func MsgBoxHookProc($uMsg,$wParam, $LParam); As Long
    ...
    If $uMsg = $HCBT_ACTIVATE Then
        ...
        ; Commented this out because it is already working properly.
        ...
        ; Code modified to use SetDlgItemTextW in place of SetDlgItemText. wstr in place of str.
        ; Enables unicode support as a result
        If ($MSGBOXEX_strOk <> "") Then  DllCall("user32.dll","long","SetDlgItemTextW","long",$wParam,"long", 1,"wstr", $MSGBOXEX_strOk)             ;~Use IDYES for VB
        If ($MSGBOXEX_strYes <> "") Then  DllCall("user32.dll","long","SetDlgItemTextW","long",$wParam,"long", 6,"wstr", $MSGBOXEX_strYes)             ;~Use IDYES for VB
        If ($MSGBOXEX_strNo <> "") Then DllCall("user32.dll","long","SetDlgItemTextW","long",$wParam,"long", 7,"wstr", $MSGBOXEX_strNo)                ;~Use IDNO for VB
        If ($MSGBOXEX_strCancel <> "") Then DllCall("user32.dll","long","SetDlgItemTextW","long",$wParam,"long", 2,"wstr", $MSGBOXEX_strCancel)    ;~Use IDCANCEL for VB
        
        ;~Change the dialog prompt text
        ; Commented this out because it is already working properly.
        ;DllCall("user32.dll","long","SetDlgItemTextW","long",$wParam,"long", 65535,"wstr", $MSGBOXEX_strPrompt)
